<header
    class="bg-white border-b border-gray-200 h-[60px] sm:h-[68px] flex items-center px-3 sm:px-5 md:px-6 gap-3 sm:gap-4 shrink-0 z-20 sticky top-0 shadow-sm">
    ...
    <div class="flex items-center gap-1.5 sm:gap-2.5 ml-auto shrink-0">
        ...
        <div x-data="{ open: false }" class="relative">
            <button @click="open = !open"
                ...
                <div
                    class="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center shrink-0 border-2 border-emerald-200 overflow-hidden">
                    @if (Auth::user()->avatar)
                ...
                <span
                    class="hidden sm:block text-sm font-medium text-gray-700 truncate max-w-[100px]">{{ Auth::user()->name }}</span>
                <svg class="w-3 h-3 text-gray-500 hidden sm:block" fill="none" stroke="currentColor"
