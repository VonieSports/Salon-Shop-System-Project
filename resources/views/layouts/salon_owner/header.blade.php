<header class="bg-white border-b border-gray-200 h-[60px] sm:h-[68px] flex items-center px-3 sm:px-5 md:px-6 lg:px-6 gap-2 sm:gap-3 md:gap-4 lg:gap-4 shrink-0 z-20 sticky top-0 shadow-sm">
    ...
    <div x-show="sidebarOpen" 
        ...
        <!-- Optional: small indicator or empty space -->
    </div>
    ...
    <div class="flex items-center gap-1.5 sm:gap-2 md:gap-2.5 lg:gap-2.5 ml-auto shrink-0">
        ...
        <!-- Chat Button -->
        <a href="{{ route('owner.chat') }}" wire:navigate 
           class="relative p-2 rounded-lg text-gray-500 hover:text-emerald-600 hover:bg-emerald-50 transition">
            ...
            @php
                try {
                    ...
                    if ($tenantId) {
                        $conversations = $chatService->getUserConversations(auth()->id(), $tenantId);
            ...
            @endphp
            @if($unreadCount > 0)
                <span class="absolute -top-0.5 -right-0.5 px-1.5 py-0.5 text-[10px] font-bold bg-red-500 text-white rounded-full min-w-[18px] text-center">
