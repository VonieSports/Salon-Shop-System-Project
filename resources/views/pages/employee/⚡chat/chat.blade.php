<div>
    ...
    <div class="flex flex-col sm:flex-row h-[calc(100vh-70px)] sm:h-[calc(100vh-80px)] md:h-[calc(100vh-90px)] bg-white rounded-none sm:rounded-2xl shadow-none sm:shadow-sm border-0 sm:border border-gray-200 overflow-hidden">
        <!-- Sidebar -->
        <div class="w-full sm:w-72 md:w-80 lg:w-96 border-b sm:border-b-0 sm:border-r border-gray-200 flex flex-col bg-gray-50/30 {{ ($selectedConversation && !$showSidebar) ? 'hidden sm:flex' : 'flex' }}">
        ...
        <div class="flex-1 flex flex-col bg-white {{ (!$selectedConversation || $showSidebar) ? 'hidden sm:flex' : 'flex' }}">
            @if($selectedConversation)
                ...
                <div class="p-2 sm:p-3 md:p-4 border-b border-gray-200 flex items-center gap-2 sm:gap-3 bg-white shrink-0 z-10">
                    <button wire:click="goBackToSidebar" 
                            class="sm:hidden p-1.5 rounded-lg text-gray-500 hover:bg-gray-100 transition flex-shrink-0">
                    ...
                    @if($selectedConversation->type == 'group')
                        <div class="w-8 h-8 sm:w-9 sm:h-10 rounded-full bg-gradient-to-r from-purple-500 to-purple-600 flex items-center justify-center text-white font-bold text-xs sm:text-sm shrink-0">
                            {{ substr($selectedConversation->name, 0, 2) }}
                        </div>
                        <div class="min-w-0 flex-1 cursor-pointer" wire:click="openMembersModal">
                            <p class="text-sm sm:text-base font-semibold text-gray-900 truncate hover:text-[#1E7A4A] transition">
                                {{ $selectedConversation->name }}
                                <span class="text-[10px] sm:text-xs text-gray-500 font-normal">({{ $selectedConversation->participants->count() }} members)</span>
                            </p>
                            <p class="text-[10px] sm:text-xs text-gray-500">Click to view members</p>
                        </div>
                        
                        <button wire:click="openMembersModal" 
                                class="p-1.5 sm:p-2 text-gray-500 hover:text-[#1E7A4A] hover:bg-emerald-50 rounded-lg transition flex-shrink-0">
                            <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    ...
                    @else
                        ...
                        @if($participant && $participant->avatar)
                            <img src="{{ Storage::url($participant->avatar) }}" 
                                 class="w-8 h-8 sm:w-9 sm:h-10 rounded-full object-cover border-2 border-gray-200 shrink-0">
                        @else
                            <div class="w-8 h-8 sm:w-9 sm:h-10 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700 font-bold text-sm sm:text-base shrink-0">
                        ...
                        <div class="relative flex-shrink-0" x-data="{ open: false }">
                            <button @click="open = !open" 
                                    class="p-1.5 sm:p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition">
                                <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            ...
                            <div x-show="open" 
                                 ...
                                 class="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-200 z-50 py-1.5">
                                
                                @if($participant)
                                    <button wire:click="toggleBlockUser({{ $participant->id }})" 
                                            wire:loading.attr="disabled"
                                            @click="open = false"
                                            class="w-full flex items-center gap-3 px-4 py-2.5 text-sm transition hover:bg-gray-50 {{ $participant->is_blocked ? 'text-green-700 hover:text-green-800' : 'text-red-700 hover:text-red-800' }}">
                                        @if($participant->is_blocked)
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                            </svg>
                                            <span>Unblock {{ $participant->name }}</span>
                                        @else
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/>
                                            </svg>
                                            <span>Block {{ $participant->name }}</span>
                                        @endif
                                    </button>
                                @endif
                                ...
                                <button wire:click="deleteConversation" 
                                        ...
                                        @click="open = false"
                                        class="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-red-700 hover:bg-red-50 transition">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                ...
                                <div class="flex {{ $message->user_id == auth()->id() ? 'justify-end' : 'justify-start' }}">
                                    <div class="max-w-[85%] sm:max-w-[75%]">
                                        <div class="flex items-end gap-1 sm:gap-2">
                                            ...
                                            @endif
                                            <div class="flex flex-col max-w-[250px] sm:max-w-[400px]">
                                                @if($message->type == 'file' && $message->file_path)
                                                    ...
                                                    @if($isImage)
                                                        <div class="bg-gray-100 rounded-lg p-2 sm:p-3 cursor-pointer hover:bg-gray-200 transition"
                                                             wire:click="previewFile('{{ $message->file_path }}', '{{ $message->file_name }}')">
                                                            <div class="flex items-center gap-1.5 sm:gap-2 text-[#1E7A4A]">
                                                                <img src="{{ Storage::url($message->file_path) }}" 
                                                                     alt="{{ $message->file_name }}"
                                                                     class="w-12 h-12 sm:w-16 sm:h-16 object-cover rounded-lg">
                                                                <div class="flex-1 min-w-0">
                                                                    <p class="text-xs sm:text-sm font-medium truncate">{{ $message->file_name }}</p>
                                                                    <p class="text-[10px] sm:text-xs text-gray-500">Click to preview</p>
                                                                </div>
                                                    ...
                                                    @else
                                                        <a href="{{ Storage::url($message->file_path) }}" 
                                                           target="_blank"
                                                           class="block bg-gray-100 rounded-lg p-2 sm:p-3 hover:bg-gray-200 transition">
                                                            <div class="flex items-center gap-1.5 sm:gap-2 text-[#1E7A4A]">
                                                                <svg class="w-8 h-8 sm:w-10 sm:h-10 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/>
                                                                </svg>
                                                                <div class="flex-1 min-w-0">
                                                                    <p class="text-xs sm:text-sm font-medium truncate">{{ $message->file_name }}</p>
                                                                    <p class="text-[10px] sm:text-xs text-gray-500">Click to open</p>
                                                                </div>
                                                                <svg class="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <div class="p-2 sm:p-3 md:p-4 border-t border-gray-200 bg-white shrink-0">
                    <form wire:submit.prevent="sendMessage" class="flex items-center gap-1.5 sm:gap-3">
                        ...
                        <button type="submit" 
                                class="p-1.5 sm:p-2 bg-[#1E7A4A] text-white rounded-xl hover:bg-[#16633c] transition flex-shrink-0"
                                wire:loading.attr="disabled">
    ...
    <!-- Modals -->
    @include('layouts.partials.new-chat-modal')
    @include('layouts.partials.group-members-modal')
    @include('layouts.partials.file-preview')
</div>