        ...
        <div class="mx-auto space-y-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl bg-[#1E7A4A]/10 flex items-center justify-center shrink-0">
                        <svg class="w-5 h-5 text-[#1E7A4A]" fill="none" stroke="currentColor" stroke-width="2"
                            viewBox="0 0 24 24">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-xl font-bold text-[#1E7A4A] mt-0.5">{{ number_format($this->stats['total_stock'])
                            }}</p>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-xl font-bold text-[#1E7A4A] mt-0.5">{{ $this->stats['available_categories'] }}
                        </p>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl bg-[#1E7A4A]/10 flex items-center justify-center shrink-0">
                        <svg class="w-5 h-5 text-[#1E7A4A]" fill="none" stroke="currentColor" stroke-width="2"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M9 13h6m-3-3v6m5.5 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                        </svg>
                    </div>
                    <div class="min-w-0">
            ...
            <div class="inline-flex bg-white border border-gray-200 rounded-xl p-1">
                <button wire:click="$set('activeTab', 'stock')"
                    class="px-4 py-2 rounded-lg text-sm font-medium transition {{ $activeTab === 'stock' ? 'bg-[#1E7A4A] text-white' : 'text-gray-600 hover:bg-gray-50' }}">Stock
                    Levels</button>
                <button wire:click="$set('activeTab', 'history')"
                    class="px-4 py-2 rounded-lg text-sm font-medium transition {{ $activeTab === 'history' ? 'bg-[#1E7A4A] text-white' : 'text-gray-600 hover:bg-gray-50' }}">Movement
                    History</button>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 p-16 text-center">
                ...
                <a href="{{ route('owner.create_product') }}"
                    class="text-[#1E7A4A] hover:underline text-sm font-medium mt-2 inline-block">Create your first
                    product</a>
            </div>
            @else
                            ...
                            <tr wire:key="product-{{ $product->id }}" class="hover:bg-gray-50/60 transition">
                                <td class="px-5 py-3.5">
                                    <div class="flex items-center gap-3 cursor-pointer"
                                        wire:click="viewProductById({{ $product->id }})">
                                        @if($product->image)
                                        <img src="{{ Storage::url($product->image) }}"
                                            class="h-10 w-10 rounded-lg object-cover shrink-0">
                                        @else
                                        <div
                                            class="h-10 w-10 bg-gray-100 rounded-lg flex items-center justify-center shrink-0">
                                        ...
                                        <div class="min-w-0">
                                            <p
                                                class="text-sm font-medium text-gray-900 hover:text-[#1E7A4A] transition truncate">
                                                {{ $product->name }}</p>
                                ...
                                <td class="px-5 py-3.5">
                                    ...
                                    <span
                                        class="px-2.5 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-700">Out
                                        of Stock</span>
                                    @elseif ($product->is_low_stock)
                                    <span
                                        class="px-2.5 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-700">Low
                                        Stock</span>
                                    @else
                                    <span
                                        class="px-2.5 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-700">In
                                        Stock</span>
                                    @endif
                                </td>
                                <td class="px-5 py-3.5 text-sm text-gray-400">{{ $product->updated_at->format('M d, Y')
                                    }}</td>
                                <td class="px-5 py-3.5">
                                    @php
                                    ...
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <button wire:click="openStockManager({{ $product->id }})"
                                            class="p-2 text-gray-400 hover:text-[#1E7A4A] hover:bg-[#1E7A4A]/10 rounded-lg transition"
                                            title="Add Stock">
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            ...
                            <tr wire:key="log-{{ $log->id }}">
                                <td class="px-5 py-3.5 text-sm text-gray-900">{{ $log->product?->name ?? 'Deleted
                                    product' }}</td>
                                <td class="px-5 py-3.5">
                                    <span
                                        class="px-2.5 py-1 text-xs font-semibold rounded-full {{ in_array($log->type, ['sale','damage']) ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700' }}">
                                        {{ ucfirst($log->type) }}
                                    </span>
                                </td>
                                <td class="px-5 py-3.5 text-sm font-semibold text-gray-900">{{ $log->quantity }}</td>
                                <td class="px-5 py-3.5 text-sm text-gray-500">{{ $log->stock_before }} → {{
                                    $log->stock_after }}</td>
                                <td class="px-5 py-3.5 text-sm text-gray-500">{{ $log->reference ?? '—' }}</td>
                                <td class="px-5 py-3.5 text-sm text-gray-400">{{ $log->created_at->format('M j, Y g:i
                                    A') }}</td>
                            </tr>
                            @endforeach
                        </tbody>
