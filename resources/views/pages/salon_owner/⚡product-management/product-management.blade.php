...
<div>
    ...
    <div class="mx-auto space-y-6">
                    ...
                    @foreach ($this->items as $item)
                        <button type="button" wire:click="viewItem({{ $item->id }})" wire:key="item-{{ $item->id }}"
                            ...
                            <div class="p-4">
                                ...
                                <p class="text-xs text-gray-400 mt-0.5">{{ $item->productCategory?->name ?? 'Uncategorized' }}</p>
        ...
        @else
            ...
            {{-- ============ DETAIL VIEW ============ --}}
            @if ($this->selectedPost && $this->selectedProduct)
                @php
                    ...
                    <div class="p-6 space-y-6">
                        <div>
                            ...
                            <div class="flex items-center gap-2 mt-2 flex-wrap">
                                @if ($variants->count() > 0)
                                    <span class="px-2.5 py-1 bg-emerald-500/15 text-emerald-400 text-xs font-semibold rounded-full">{{ $variants->count() }} variants</span>
                                @endif
                                <span class="text-gray-400 text-sm">{{ $post->productCategory?->name ?? 'Uncategorized' }}</span>
                            </div>
                        </div>
                            ...
                            <div class="lg:col-span-5 space-y-5">
                                ...
                                <div class="bg-white/5 rounded-2xl p-5 space-y-3">
                                    ...
                                    <div class="grid grid-cols-2 gap-y-2 text-sm">
                                        ...
                                        <span class="text-right">{{ $post->productCategory?->name ?? 'Uncategorized' }}</span>
                                        <span class="text-gray-500">SKU</span>
                                        <span class="text-right font-mono text-xs">{{ $product->sku }}</span>
                            ...
                            <div class="lg:col-span-7">
                                <div class="flex items-center justify-between mb-3">
                                    <h3 class="text-sm font-bold text-gray-300 uppercase tracking-wide">
                                        {{ $variants->count() > 0 ? 'Variants' : 'Stock' }}
                                    </h3>
                                    <span class="text-sm text-gray-400">
                                        Total: <span class="font-bold text-white">{{ $product->stock }}</span> in stock
                                ...
                                @if ($variants->isEmpty())
                                    <div class="bg-white/5 rounded-2xl p-5">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <span class="text-xl font-bold">${{ number_format($product->selling_price, 2) }}</span>
                                        </div>
                                            ...
                                            <div class="bg-white/5 hover:bg-white/[0.07] rounded-xl p-4 border border-white/10 transition">
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    <div class="text-right shrink-0">
                                                        <p class="text-sm font-bold">${{ number_format($product->selling_price + $variant->price_adjustment, 2) }}</p>
                                                        <p class="text-xs {{ $variant->stock <= $product->low_stock_alert ? 'text-red-400' : 'text-gray-500' }}">
                                                            {{ $variant->stock }} in stock
                                                            @if ($variant->stock <= $product->low_stock_alert) &bull; low @endif
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
            ...
            @else
                <div class="bg-white rounded-2xl border border-gray-100 p-16 text-center">
                    ...
                    <button wire:click="closeItem" class="mt-3 text-sm font-medium text-[#1E7A4A] hover:underline">Back to products</button>
                </div>
            @endif
