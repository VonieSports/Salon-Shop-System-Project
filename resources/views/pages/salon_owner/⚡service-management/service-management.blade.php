<div class="min-h-screen bg-gray-50">
    <div class="mx-auto space-y-6">
        ...
        onclick="return confirm('Archive {{ count($selectedIds) }} selected service(s)? You can restore them later from the Archive page.')"
        class="ml-auto inline-flex items-center gap-1.5 px-3 py-1.5 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition text-xs font-semibold">
                ...
                <div class="divide-y divide-gray-100">
                    @foreach ($this->items as $item)
                        <div wire:key="item-{{ $item->id }}" class="flex items-center gap-4 px-5 py-3.5 hover:bg-gray-50 transition">
                            ...
                            <button type="button" wire:click="viewItem({{ $item->id }})"
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-xs text-gray-400 mt-0.5 truncate">{{ $item->serviceCategory?->name ?? 'Uncategorized' }} &bull; {{ $item->created_at->format('M j, Y') }}</p>
                                </div>

                                <span class="text-sm font-bold text-gray-900 shrink-0">${{ number_format($item->price ?? 0, 2) }}</span>

                                <span class="px-2 py-0.5 text-[10px] font-semibold rounded-full shrink-0 {{ $item->status === 'published' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">
                                    {{ ucfirst($item->status) }}
                                </span>

                                <svg class="w-5 h-5 text-gray-300 group-hover:text-[#1E7A4A] transition shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    @include('layouts.partials.service-detail-modal')
</div>